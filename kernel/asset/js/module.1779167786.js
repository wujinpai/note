                ...
                if (JSON.stringify(dir) !== '{}') {
                    ...
                    _media.forEach(pic => {
                        ...
                        if (pic.url) {
                            ...
                            pics += `<div class="${origFile.endsWith('GIF.webp') ? 'gif' : getSupportFormat(pic[1])}${(piclen == 9 && _media.length > 9) ? ' more' : (piclen > 9 ? ' annexh' : '')}${pic[0].includes('Enc') ? ' enc' : ''}"><img loading="lazy" decoding="async" src="/uploads/${dir.dir + origFile}" data-url="${dir.dir + '/' + pic[0] + '.' + pic[1]}" data-info='${pic[2]}' onerror="this.src='/kernel/asset/img/default-image.webp'; this.onerror=null;" alt="${JSON.parse(pic[2])[0][1]}">${_num}</div>`;
                        }
                    });
                    piclen = 0;
                }
