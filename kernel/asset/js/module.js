    ...
    function loadmedia(current) {
        if (current.classList.contains('video')) {
            const source = createElement({
                type: 'source',
                attributes: {
                    src: "/uploads/" + current.children[0].dataset.url,
                    type: 'video/mp4'
                }
            })
            const video = createElement({
                type: 'video', attributes: {
                    poster: current.children[0].src,
                    controls: false
                }, children: [source]
            })
            video.setAttribute('data-info', current.children[0].dataset.info)
            ...
            picture.src = "/uploads/" + current.children[0].dataset.url;
            picture.dataset.info = current.children[0].dataset.info;
            tip.style.opacity = '(1)';
            return picture;
        }
    }
...
function createSelection(options = {}, callback = () => { }) {
    ...
    if (!defaultValue && update_select_text) {
        const _f = popup.childNodes[0].disabled ? popup.childNodes[1] : popup.childNodes[0];
        ...
        .then(async data => {
            ...
            async function processItem(item) {
                ...
                if (JSON.stringify(dir) !== '{}') {
                    ...
                    _media.forEach(pic => {
                        piclen++;
                        const file = "/thum-" + pic[0] + (pic[0].includes('Enc') ? ".png" : ".webp");
                        let _num = '';
                        if (piclen == 9 && _media.length - 9 > 0) {
                            _num = `<span>剩余${_media.length - 9}张</span>`;
                        }
                        pics += `<div class="${file.endsWith('GIF.webp') ? 'gif' : getSupportFormat(pic[1])}${(piclen == 9 && _media.length > 9) ? ' more' : (piclen > 9 ? ' annexh' : '')}${pic[0].includes('Enc') ? ' enc' : ''}"><img loading="lazy" src="/uploads/${dir.dir + file}" data-url="${dir.dir + '/' + pic[0] + '.' + pic[1]}" data-info='${pic[2]}' onerror="this.src='/kernel/asset/img/default-image.webp'; this.onerror=null;" alt="${JSON.parse(pic[2])[0][1]}">${_num}</div>`;
                    });
                    piclen = 0;
                }
                ...
                const html = `
                    <div class="post${item.hidden == 1 ? (item.is_hidden == 1 ? '' : ' post_hidden') : ''} ${item.pin ? ' post_pin' : ''}" tabindex="0" data-id="${item.id}" data-i="${index++}">
                        ...
                        <div class="content" ${item.hidden == 1 ? 'style="display:none"' : ''}><span class="tag icon-topic">${item.tag}</span>${item.content}</div>
                        <div class="annex">
                            ${pics}
                        </div>
                        <div class="meta">
                ...
                setTimeout(async () => {
                    ...
                    if (content.innerText.includes('Encryption=')) {
                        ...
                        content.innerHTML = await DecryptText(content.innerText);
            ...
            if (window.innerWidth < 881) {
                PostDOMList.forEach(item => {
                    ...
                    item.classList.contains('post') && !item.classList.contains('post_hidden') ? foldPost(item.querySelector('.content')) : '';
                })
            } else {
                flowFix({
                    ...
                    item.childNodes.forEach(i => {
                        i.classList.contains('post') && !item.classList.contains('post_hidden') ? setTimeout(() => { foldPost(i.querySelector('.content')) }, 0) : '';
                    })
                })
            }
            if (_down) {
