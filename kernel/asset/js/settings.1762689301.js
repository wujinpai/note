function settingsPanel() {
    ...
    async function getConfig() {
        ...
        const info = new DOMParser().parseFromString(`
            ...
            <div class="expload">
                <span>建议或反馈<br>puffton@mail.com</span>
                <br>
                <br>
...
function PostEditorPanel(mode = 'add') {
    ...
    const editHtml = `
        <div id="editor" data-mode="add" class="post editor post_pin_disable" tabindex="0" style="border: 1px dashed #999;">
            ...
            <div class="content"><span id="editor_tag" class="tag icon-topic" style='float:left' contenteditable="true" data-placeholder="${getDomFromId('menu').dataset.t}"></span><div id="editor_content" contenteditable="true" style='outline:none;word-break: break-all;' data-placeholder="请输入内容..."></div>
            </div>
    ...
    sendpost.onclick = async () => {
        ...
        for (let i = 0; i < mediaLen; i++) {
            ...
            const _temp = dom.children[0].dataset.url.match(/^(?:(.*)\/)?([^\/]+?)\.([^.\/]+)$/);
            files.push([_temp[1], _temp[2], _temp[3], dom.children[0].getAttribute('data-info').replace(/&quot;/g, '"')]);
            if (dom.classList.contains('video')) {
                files[i].push(dom.children[0].src);
            }
        }
        ...
        let E_content = removeAllAttributes(sanitizeContent(getDomFromId('editor_content').innerHTML, true)).trim();
